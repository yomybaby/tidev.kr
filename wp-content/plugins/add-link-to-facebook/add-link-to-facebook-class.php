			...
			if ($user_ID && !self::Is_excluded($post) && !WPAL2Int::social_in_excerpt($user_ID)) {
				...
				if($fb_comments){
					...
					foreach ($fb_comments->data as $fb_comment){
						if($fb_comment->from->id != '100004739790673'){
							//if (get_user_meta($user_ID, c_al2fb_meta_fb_comments_nolink, true) == 'author') {
 								$link = WPAL2Int::Get_fb_profilelink($fb_comment->from->id);
 								$commnets .= '<a href="' . $link . '" rel="nofollow">' . htmlspecialchars($fb_comment->from->name, ENT_QUOTES, $charset) . '</a>(' .date('m-d H:i', strtotime($fb_comment->created_time)+$tz_off).') : ';
 							//}
							$commnets.= $fb_comment->message;
							$commnets.='<br/>';
						}
				...
				if(!empty($commnets)){
					//$commnets = '<div class="al2fb commnets">'.$comments.'</div>';
					$likers.=$commnets;
				}
				if($likers)
					$likers = '<br/><div style="background-color:#f3f3f3; padding:5px;"><div style="background-color : #fff;"><a href="http://tidev.kr/fb"><strong>Facebook Group</strong></a>\'s likes and commnets</div>'.$likers.'</div>';
				// comment import end
			}
			return $likers;
		}
